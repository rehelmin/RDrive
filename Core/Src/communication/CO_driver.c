...
void CO_CANsetConfigurationMode(int32_t CANbaseAddress){
    ...
    if (CANbaseAddress > 2) {
        SET_BIT(CANbaseAddress, CAN_MCR_INRQ);
    }
    else if (CANbaseAddress == 1) {
        handle_ = hcan1;
    }

    if (handle_.State > HAL_CAN_STATE_RESET) {
        ...
        uint16_t                CANbitRate)
{
    ...
    if (CANbaseAddress == 1) {
        CANmodule->CANbaseAddress = (&hcan1)->Instance->MCR;
    }
    ...
    // PRESCALER IS CURRENTLY SET TO ASSUME 1MBaud!!
    ...
    hcan1.Init.Prescaler = 2;  // 21 TQ's
...
CO_ReturnError_t CO_CANsend(CO_CANmodule_t *CANmodule, CO_CANtx_t *buffer){
    ...
    if(HAL_CAN_GetTxMailboxesFreeLevel(&hcan1) > 0 && CANmodule->CANtxCount == 0){
        CANmodule->bufferInhibitFlag = buffer->syncFlag;
        HAL_CAN_AddTxMessage(&hcan1, &header, buffer->data, &retTxMailbox);
    }
...
/******************************************************************************/
void CO_CANinterrupt(CO_CANmodule_t *CANmodule, uint32_t interrupt_type){

    /* receive interrupt */
    if(interrupt_type == RECEIVE){
        ...
        if (HAL_CAN_GetRxFifoFillLevel(&hcan1, CAN_RX_FIFO0) > 0) {
            HAL_CAN_GetRxMessage(&hcan1, CAN_RX_FIFO0, &header, rcvMsg.data);
        } else if (HAL_CAN_GetRxFifoFillLevel(&hcan1, CAN_RX_FIFO1) > 0) {
            HAL_CAN_GetRxMessage(&hcan1, CAN_RX_FIFO1, &header, rcvMsg.data);
        }

        rcvMsg.ident = header.StdId;
        rcvMsg.DLC = header.DLC;
